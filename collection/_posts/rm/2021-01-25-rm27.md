...
`Def.`{:.info} 
Given a set $X$, we call an element of $\sigma$-algebra a **measurable set** and we call $X$ a **measurable space**(M.S.)
{:.info}
...
$$ \text{Given M.S. } (X, \mathfrak{M}) \text{, T.S. } (Y, T_Y) \text{ and two measurable functions } u: X \rightarrow \mathbb{R}, v: Y \rightarrow \mathbb{R},   \nonumber $$
...
$$ \text{By above proposition, for M.S. } (X, \mathfrak{M}), \nonumber $$
