...
$x_i \in A, y_i \in B$.
1.  * ($\Rightarrow$): pick any $z \in A$. Define $g_z: B \rightarrow A$ s.t. $aI\\[b \in \text{Im}f \\]+zI\\[b \notin \text{Im}f$. Then, $g_z$ is well-defined because if $b=f(a)=f(a')$ then $a=a'$ since $f$ is injetive.
    * ($\Leftarrow$):  If $f(x_1) = f(x_2)$, then $g\cdot f(x_1) = x_1 = g\cdot f(x_2) = x_2$.
2.  * ($\Rightarrow$): Suppose $f$ is surjective and clarify for each $y\in B$, assign $x\in A$ s.t. $y=f(x)$. Then, we can define $g: B \rightarrow A \ ( y \mapsto x)$. NB; only assume $f$ is surjective, so existence of $g$ requires axiom of choice. 
